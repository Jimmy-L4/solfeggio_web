...
define('modules/Comments/src/CommentSpaceManager',['modules/Edition/src/ElementManager',
...
define('modules/core/src/PitchClass',[
	'utils/NoteUtils'
	], function(NoteUtils) {
	/**
...
define('modules/core/src/ChordModel',[
	 ...
	 */
	define('modules/core/src/ChordManager',[
...
define('modules/LSViewer/src/BarWidthManager',['modules/core/src/SongBarsIterator', 'modules/core/src/SectionBarsIterator'], function(SongBarsIterator, SectionBarsIterator) {
	/**
    * 
...
define('modules/FileEdition/src/FileEditionController',[
  ...
  FileEditionController.prototype.initSubscribeSaveAS = function (
    first_argument
  ) {
    var content =
  ...
  FileEditionController.prototype.importMusicCSLJSON = function (JSONSong) {
    if (typeof JSONSong === 'undefined') {
      throw (
        'FileEditionController - importMusicCSLJSON File imported is not defined ' +
        JSONSong
      )
    }
    ...
    $.publish(
      'ToHistory-add',
      'Open MusicCSLJson - ' + this.songModel.getTitle()
    )
  ...
  FileEditionController.prototype.importMusicXML = function (musicXMLSong) {
    if (typeof musicXMLSong === 'undefined') {
      throw (
        'FileEditionController - importMusicXML File imported is not defined ' +
        musicXMLSong
      )
    }
   ...
   */
  FileEditionController.prototype.promptFile = function (
    title,
    path,
    extension
  ) {
  ...
  FileEditionController.prototype.exportPNG = function () {
    ...
    this.promptFile(
      this.songModel.getTitle() + '.png',
      this.viewer.canvas.toDataURL('image/png')
    )

    // reduce resolution
    this.viewer.typeResize = 'fluid'
    this.viewer.canvas.width = Math.ceil(
      this.viewer.canvas.width / resolutionRatio
    )
    this.viewer._resize(this.viewer.canvas.width)
    this.viewer.draw(this.songModel)
  }
  ...
  FileEditionController.prototype.exportLeadsheetJSON = function () {
    ...
    var data =
      'text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(JSONSong))
    var export_link = $('<a>', {
  ...
  FileEditionController.prototype.save = function (
    ...
    if (this.saveFn !== undefined) {
      if (
        showConfirm &&
        !confirm('你确定要提交 ' + this.songModel.getTitle() + '这首曲目?')
      ) {
        return
      }
      var self = this
      ...
      this.saveFn(JSONSong, songId, { derivedId: derivedId }, function (data) {
        ...
        if (data.error) {
          UserLog.logAutoFade('提交失败', data.msg)
        } else {
          UserLog.logAutoFade('提交成功', '成功上传至服务器！')
        }
  ...
  FileEditionController.prototype.saveToServer = function (
    newLeadsheet,
    showConfirm,
    derived
  ) {
    ...
    if (this.saveFn !== undefined) {
      if (
        ...
        !confirm(
          '你确定要暂存' +
            this.songModel.getTitle() +
            '这首曲目?暂存后仅保存当前曲目，并不提交，请于截止日期前提交！'
        )
      ) {
        return
      }
...
define('text!modules/MainMenu/src/MainMenuTemplate.html',[],function () { return '<div id="view_main_menu">\n\t<div id="main_menu_first_level"></div>\n\t<div id="main_menu_second_level"></div>\n</div>';});
...
define('modules/Audio/src/AudioController',['jquery', 'pubsub', 'modules/Audio/src/AudioContext'], function($, pubsub, AudioContext) {
	/**
...
define('modules/Audio/src/BarTimesManager',['modules/core/src/SongBarsIterator'],
...
define('modules/Audio/src/AudioDrawer',['modules/Audio/src/BarTimesManager',
	'modules/core/src/SongBarsIterator',
...
define('LJS',[
  ...
  var snglNotesManager = singleton(function (args) {
    var songModel = args[0]
    var viewer = args[1]
    return new NoteSpaceManager(
      snglNotesCursor.getInstance(songModel),
      viewer,
      'NotesCursor',
      null,
      true,
  ...
  function loadPlayer(options, songModel) {
    ...
    playerViewOptions.displaySwitch =
      useAudio && useMidi && options.audio.audioFile //if audioFile is not defined, we do not load displayTypeSwitch
    playerViewOptions.tempo = !isNaN(songModel.getTempo())
      ? songModel.getTempo()
      : null
    ...
    new NoteSpaceManager(
      snglPlayerCursor.getInstance(songModel),
      viewer,
      'PlayerCursor',
      '#0AA000',
      false,
  ...
  LJS.init = function (MusicCSLJSON, params) {
    ...
    if (useViewer) {
      ...
      viewer = loadViewer(
        params.viewer.HTMLElement,
        params.viewer.viewOptions,
        songModel
      )
    }
    ...
    if (useEdition) {
      var menuHTML =
        params.edition.menu !== undefined
          ? params.edition.menu.HTMLElement
          : false
      var editionModule = loadEdition(
        viewer,
        songModel,
        menuHTML,
        params.edition
      ...
      var fileEdition = new FileEdition(
        songModel,
        viewer,
        params.edition.saveFunction,
        {
